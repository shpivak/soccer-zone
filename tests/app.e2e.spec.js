...
test('management controls are separated and saturday a can still start empty', async ({ page }) => {
  ...
  await expect(page.getByTestId('dataset-select')).toBeDisabled()
  await expect(page.getByTestId('reset-dataset')).toBeDisabled()
...
test('games, scoring events, standings, editing, deleting, undo, and stats updates work end to end', async ({
  ...
  await page.getByTestId('nav-stats').click()
  await expect(page.getByRole('heading', { name: 'סטטיסטיקות כלל הטורנירים - שבת א׳' })).toBeVisible()
