import defaultPlayersSource from '../../mock_data/players.json' with { type: 'json' }
import tournamentOneSource from '../../mock_data/tournaments/tournament-1.json' with { type: 'json' }
import tournamentTwoSource from '../../mock_data/tournaments/tournament-2.json' with { type: 'json' }
import tournamentThreeSource from '../../mock_data/tournaments/tournament-3.json' with { type: 'json' }
import tournamentFourSource from '../../mock_data/tournaments/tournament-4.json' with { type: 'json' }
import tournamentFiveSource from '../../mock_data/tournaments/tournament-5.json' with { type: 'json' }
...
const tournamentPlayers = defaultPlayersSource.map((player) => ({
  ...withPlayerRoles(player, Number(String(player.id).replace(/\D/g, '')) || 0),
...
const regularLeagueTeams = [
  { id: 'regular-team-1', name: 'נשרים', color: 'black', players: ['regular-p1', 'regular-p2', 'regular-p3', 'regular-p13'] },
  { id: 'regular-team-2', name: 'זאבים', color: 'yellow', players: ['regular-p4', 'regular-p5', 'regular-p6', 'regular-p14'] },
  { id: 'regular-team-3', name: 'דרקונים', color: 'pink', players: ['regular-p7', 'regular-p8', 'regular-p9', 'regular-p15'] },
  { id: 'regular-team-4', name: 'אריות', color: 'blue', players: ['regular-p10', 'regular-p11', 'regular-p12', 'regular-p16'] },
]
...
const friendlyPlayers = ['אדם לוי', 'ניר דיין', 'עומר זיו', 'שקד עזר', 'דניאל רון', 'יואב הדר'].map((name, index) => ({
...
export const defaultLeagues = [
  { id: 'tournament-1', name: 'שישי צהריים', type: LEAGUE_TYPES.tournament, seasonLabel: '2026', teams: [], allowRosterEdits: false },
  { id: 'tournament-2', name: 'שבת A', type: LEAGUE_TYPES.tournament, seasonLabel: '2026', teams: [], allowRosterEdits: false },
  { id: 'tournament-3', name: 'שבת B', type: LEAGUE_TYPES.tournament, seasonLabel: '2026', teams: [], allowRosterEdits: false },
  { id: regularLeagueId, name: 'ליגת סוקרזון 5', type: LEAGUE_TYPES.regular, seasonLabel: '2026', teams: clone(regularLeagueTeams), allowRosterEdits: false },
  { id: friendlyLeagueId, name: 'סטטיסטיקת ידידות', type: LEAGUE_TYPES.friendly, seasonLabel: '2026', teams: [], allowRosterEdits: false },
]
